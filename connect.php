...
$pageTitle = 'Stripe Connection - Stripe Dashboard';
$errorMessage = null;
$successMessage = null;
...
<div class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
    ...
    <div class="flex items-center gap-3">
        <a href="index.php"
            class="inline-flex items-center gap-2 bg-white hover:bg-slate-50 text-slate-700 border border-slate-300 text-xs font-semibold px-3.5 py-2 rounded-lg shadow-xs transition">
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-xs p-6 sm:p-8">
            ...
            <form method="POST" action="connect.php" class="space-y-5">
                ...
                <div>
                    ...
                    <div class="relative">
                        <input id="stripe_secret_key" name="stripe_secret_key" type="password" required
                            placeholder="sk_test_..." value="<?= htmlspecialchars($currentSecret ?? '') ?>"
                    ...
                    <p class="mt-1 text-2xs text-slate-400">
                        Use a test secret key (<code class="bg-slate-100 px-1 py-0.5 rounded">sk_test_...</code>) or
                        restricted key (<code class="bg-slate-100 px-1 py-0.5 rounded">rk_test_...</code>).
                    </p>
                </div>
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-xs p-6">
            <h3
                class="text-sm font-bold text-slate-900 mb-4 pb-3 border-b border-slate-100 flex items-center justify-between">
                <span>Session Status</span>
                <?php if ($isConnected): ?>
                <span
                    class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-2xs font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200">
                    <span class="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>
                    Active
                </span>
                <?php else: ?>
                <span
                    class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-2xs font-semibold bg-slate-100 text-slate-600 border border-slate-200">
                    Disconnected
                </span>
                <?php endif; ?>
            </h3>
            ...
            <div class="space-y-3 text-xs">
                <div>
                    ...
                    <span class="font-semibold text-slate-800 uppercase text-xs"><?= htmlspecialchars($stripeMode) ?>
                        mode</span>
                </div>
                <div>
                    <span class="text-slate-400 font-medium block">Secret Key</span>
                    <span
                        class="font-mono text-2xs text-slate-700 bg-slate-100 px-2 py-1 rounded block truncate mt-0.5">
                ...
                <div class="pt-4 border-t border-slate-100">
                    <form method="POST" action="connect.php"
                        onsubmit="return confirm('Are you sure you want to disconnect your Stripe key from this session?');">
                        <input type="hidden" name="action" value="disconnect">
                        <button type="submit"
                            class="w-full inline-flex items-center justify-center gap-1.5 py-2 px-3 rounded-lg text-xs font-semibold text-rose-700 bg-rose-50 hover:bg-rose-100 border border-rose-200 transition">
            ...
            <div class="text-xs text-slate-500 space-y-2">
                ...
                <p class="text-slate-400 text-2xs">Enter your secret key on the left to start inspecting Stripe
                    customers, transactions, and subscriptions.</p>
            </div>
            <?php endif; ?>
        </div>
        ...
        <div class="bg-stripe-50 border border-stripe-100 rounded-2xl p-6 text-xs text-slate-700">
            <h4 class="font-bold text-stripe-900 mb-1.5 flex items-center gap-1.5">
                <svg class="w-4 h-4 text-stripe-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <p class="text-slate-600 leading-relaxed mb-3">
                You can find your test secret key (<code
                    class="bg-white/80 px-1 py-0.5 rounded text-2xs font-mono">sk_test_...</code>) in your Stripe
                Dashboard developers portal.
            </p>
            <a href="https://dashboard.stripe.com/test/apikeys" target="_blank" rel="noreferrer"
                class="inline-flex items-center gap-1 text-stripe-600 font-semibold hover:underline">
