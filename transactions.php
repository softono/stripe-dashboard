...
$pageTitle = 'Transactions - Stripe TestKit';
...
<div class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
    ...
    <?php if ($customerFilter): ?>
        <div class="flex items-center gap-2 bg-stripe-50 border border-stripe-200 px-3.5 py-2 rounded-xl text-xs text-stripe-800 shadow-xs">
            ...
            <a href="transactions.php" class="ml-2 inline-flex items-center px-1.5 py-0.5 rounded bg-stripe-200 text-stripe-900 hover:bg-stripe-300 font-bold transition" title="Clear customer filter">
                &times; Clear
            </a>
        </div>
    <?php endif; ?>
...
<div class="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden">
    ...
    <?php if (empty($charges)): ?>
        <div class="p-16 text-center text-slate-500">
            ...
            <?php if ($customerFilter): ?>
                <div class="mt-4">
                    <a href="transactions.php" class="text-xs font-semibold text-stripe-600 hover:underline">Clear customer filter</a>
                </div>
            <?php endif; ?>
        </div>
    <?php else: ?>
                        ...
                        <tr class="hover:bg-slate-50/75 transition group">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-1.5 font-mono text-xs text-slate-700 font-medium">
                                    ...
                                    <button type="button" onclick="copyToClipboard('<?= htmlspecialchars($charge->id) ?>', this)" class="text-slate-400 hover:text-slate-700 p-0.5 rounded transition" title="Copy Charge ID">
                            ...
                            <td class="px-6 py-4">
                                ...
                                <?php if ($charge->amount_refunded > 0): ?>
                                    <div class="text-2xs text-rose-600 font-medium mt-0.5">
                                        -<?= htmlspecialchars(formatCurrency($charge->amount_refunded, $charge->currency)) ?> refunded
                                    </div>
                                <?php endif; ?>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                ...
                                <?php if ($charge->disputed): ?>
                                    <span class="inline-flex items-center px-1.5 py-0.5 rounded text-2xs font-bold bg-red-100 text-red-800 ml-1">Disputed</span>
                                <?php endif; ?>
                            </td>
                            ...
                            <td class="px-6 py-4 text-xs">
                                ...
                                <?php if (!empty($charge->customer)): ?>
                                    <div class="flex items-center gap-1.5 mt-0.5">
                                        <a href="customer-detail.php?id=<?= urlencode((string)$charge->customer) ?>" class="text-2xs font-mono text-stripe-600 hover:underline font-semibold" title="View Customer Profile">
                                            <?= htmlspecialchars((string)$charge->customer) ?>
                                        </a>
                                        <?php if ($customerFilter !== $charge->customer): ?>
                                            <a href="transactions.php?customer=<?= urlencode((string)$charge->customer) ?>" class="text-3xs text-slate-400 hover:text-slate-700 bg-slate-100 px-1 py-0.2 rounded" title="Filter by this customer">
                                                filter
                                            </a>
                                        <?php endif; ?>
                                    </div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-xs">
                                <div class="flex items-center justify-end gap-2">
                                    <?php if (!empty($charge->receipt_url)): ?>
                                        <a href="<?= htmlspecialchars($charge->receipt_url) ?>" target="_blank" rel="noreferrer" class="text-xs text-slate-600 hover:text-slate-900 underline" title="View customer receipt">
                                            Receipt
                                        </a>
                                    <?php endif; ?>
                                    <a href="https://dashboard.stripe.com/test/payments/<?= htmlspecialchars($charge->id) ?>" target="_blank" rel="noreferrer" class="inline-flex items-center gap-1 text-stripe-600 hover:text-stripe-700 font-medium hover:underline">
                                        Stripe
