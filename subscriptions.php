...
$pageTitle = 'Subscriptions - Stripe TestKit';
...
<div class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
    ...
    <?php if ($customerFilter): ?>
        <div class="flex items-center gap-2 bg-stripe-50 border border-stripe-200 px-3.5 py-2 rounded-xl text-xs text-stripe-800 shadow-xs">
            <span class="text-slate-500">Filtered by Customer:</span>
            <a href="customer-detail.php?id=<?= urlencode($customerFilter) ?>" class="font-mono font-bold text-stripe-700 hover:underline flex items-center gap-1">
                <?= htmlspecialchars($customerFilter) ?>
                <svg class="w-3 h-3 text-stripe-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg>
            </a>
            <a href="<?= buildPageUrl(['customer' => null]) ?>" class="ml-2 inline-flex items-center px-1.5 py-0.5 rounded bg-stripe-200 text-stripe-900 hover:bg-stripe-300 font-bold transition" title="Clear customer filter">
                &times; Clear
            </a>
        </div>
    <?php endif; ?>
...
<div class="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden">
    ...
    <div class="px-6 py-3.5 bg-slate-50 border-b border-slate-200 flex flex-wrap items-center justify-between gap-3 text-xs text-slate-600">
        <div class="flex items-center gap-2">
            ...
            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs bg-slate-200 text-slate-700">Status: <?= ucfirst($status) ?></span>
            <?php if ($customerFilter): ?>
                <span class="inline-flex items-center px-2 py-0.5 rounded text-xs bg-stripe-100 text-stripe-700">Customer: <?= htmlspecialchars($customerFilter) ?></span>
            <?php endif; ?>
        </div>
    ...
    <?php if (empty($subscriptions)): ?>
        <div class="p-16 text-center text-slate-500">
            ...
            <?php if ($customerFilter): ?>
                <div class="mt-4">
                    <a href="<?= buildPageUrl(['customer' => null]) ?>" class="text-xs font-semibold text-stripe-600 hover:underline">Clear customer filter</a>
                </div>
            <?php endif; ?>
        </div>
    <?php else: ?>
                        ...
                        <tr class="hover:bg-slate-50/75 transition group">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-1.5 font-mono text-xs text-slate-700 font-medium">
                                    ...
                                    <button type="button" onclick="copyToClipboard('<?= htmlspecialchars($sub->id) ?>', this)" class="text-slate-400 hover:text-slate-700 p-0.5 rounded transition" title="Copy Subscription ID">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>
                                    </button>
                                </div>
                                <?php if ($sub->cancel_at_period_end): ?>
                                    <div class="inline-flex items-center gap-1 text-2xs text-amber-700 font-semibold bg-amber-50 border border-amber-200 px-1.5 py-0.5 rounded mt-1">
                            ...
                            <td class="px-6 py-4 text-xs">
                                ...
                                ?>
                                <a href="customer-detail.php?id=<?= urlencode($custId) ?>" class="font-semibold text-slate-800 hover:text-stripe-600 transition block">
                                    <?= htmlspecialchars($custName) ?>
                                </a>
                                <div class="flex items-center gap-1.5 font-mono text-2xs text-slate-400 mt-0.5">
                                    <a href="customer-detail.php?id=<?= urlencode($custId) ?>" class="text-stripe-600 hover:underline">
                                        <?= htmlspecialchars($custId) ?>
                                    </a>
                                    <?php if ($customerFilter !== $custId): ?>
                                        <a href="subscriptions.php?customer=<?= urlencode($custId) ?>" class="text-3xs text-slate-400 hover:text-slate-700 bg-slate-100 px-1 py-0.2 rounded" title="Filter by this customer">
                                            filter
                                        </a>
                                    <?php endif; ?>
                                </div>
                            ...
                            <td class="px-6 py-4 text-xs text-slate-600 whitespace-nowrap">
                                ...
                                <div class="text-slate-400 text-2xs">to <?= htmlspecialchars(formatDate($sub->current_period_end ?? null)) ?></div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-xs">
                                <a href="https://dashboard.stripe.com/test/subscriptions/<?= htmlspecialchars($sub->id) ?>" target="_blank" rel="noreferrer" class="inline-flex items-center gap-1 text-stripe-600 hover:text-stripe-700 font-medium hover:underline">
                                    Stripe
