...
$pageTitle = 'Customers - Stripe Dashboard';
...
<div class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
    <div>
        ...
        <p class="text-sm text-slate-500 mt-1">Live customer accounts retrieved via direct Stripe API cursor pagination.
        </p>
    </div>
    ...
    <div class="flex items-center gap-3">
        <form method="GET" action="customers.php" class="relative flex items-center">
            ...
            <div class="relative w-full max-w-xs">
                ...
                <?php if ($searchQuery): ?>
                <a href="customers.php?limit=<?= $limit ?>"
...
<div class="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden">
    ...
    <div
        ...
        <div class="flex items-center gap-2">
            <span>Showing <strong><?= count($customers) ?></strong>
                <?= $searchQuery ? 'search results' : 'customers on this page' ?></span>
            <?php if ($searchQuery): ?>
            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs bg-stripe-100 text-stripe-700">Filter:
        ...
        <table class="min-w-full divide-y divide-slate-200 text-left text-sm">
            ...
            <tbody class="divide-y divide-slate-100 bg-white">
                ...
                <tr class="hover:bg-slate-50/75 transition group">
                    <!-- Customer Name & ID -->
                    <td class="px-6 py-4">
                        <a href="customer-detail.php?id=<?= urlencode($cust->id) ?>"
                            class="font-semibold text-slate-900 hover:text-stripe-600 transition flex items-center gap-1.5">
                            <?= htmlspecialchars($cust->name ?? 'Unnamed Customer') ?>
                            <svg class="w-3.5 h-3.5 opacity-0 group-hover:opacity-100 text-stripe-500 transition"
                        ...
                        <div class="flex items-center gap-1.5 font-mono text-xs text-slate-400 mt-0.5">
                            <a href="customer-detail.php?id=<?= urlencode($cust->id) ?>"
                                class="hover:text-stripe-600 hover:underline">
                                <?= htmlspecialchars($cust->id) ?>
                            </a>
                            <button type="button" onclick="copyToClipboard('<?= htmlspecialchars($cust->id) ?>', this)"
                    ...
                    <td class="px-6 py-4 text-xs">
                        <span
                            class="font-semibold <?= ($cust->balance ?? 0) < 0 ? 'text-emerald-700' : (($cust->balance ?? 0) > 0 ? 'text-rose-700' : 'text-slate-700') ?>">
                            <?= htmlspecialchars(formatCurrency($cust->balance ?? 0, $cust->currency ?? 'usd')) ?>
                        </span>
                        <span
                            class="text-slate-400 text-2xs uppercase ml-1"><?= htmlspecialchars($cust->currency ?? 'usd') ?></span>
                    </td>
                    ...
                    <td class="px-6 py-4 text-xs">
                        ...
                        <span
                            class="inline-flex items-center px-2 py-0.5 rounded text-2xs font-semibold bg-rose-50 text-rose-700 border border-rose-200">Delinquent</span>
                        <?php else: ?>
                        <span
                            class="inline-flex items-center px-2 py-0.5 rounded text-2xs font-medium bg-slate-100 text-slate-600">Good
                            Standing</span>
                        <?php endif; ?>
                    </td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-xs">
                        <div class="flex items-center justify-end gap-1.5">
                            ...
                            <a href="customer-detail.php?id=<?= urlencode($cust->id) ?>"
                                class="inline-flex items-center gap-1 bg-white hover:bg-slate-50 text-slate-700 border border-slate-200 px-2.5 py-1 rounded-md text-xs font-semibold shadow-2xs hover:border-slate-300 transition"
                                title="View Customer Full Detail">
                                Detail
                            </a>

                            <!-- Filter Transactions for this Customer -->
                            <a href="transactions.php?customer=<?= urlencode($cust->id) ?>"
                                class="inline-flex items-center gap-1 bg-stripe-50 hover:bg-stripe-100 text-stripe-700 border border-stripe-200 px-2.5 py-1 rounded-md text-xs font-semibold transition"
                                title="Filter Transactions for this customer">
                                Charges
                            </a>

                            <!-- Filter Subscriptions for this Customer -->
                            <a href="subscriptions.php?customer=<?= urlencode($cust->id) ?>"
                                class="inline-flex items-center gap-1 bg-purple-50 hover:bg-purple-100 text-purple-700 border border-purple-200 px-2.5 py-1 rounded-md text-xs font-semibold transition"
                                title="Filter Subscriptions for this customer">
                                Subs
                            </a>

                            <!-- Stripe External Link -->
                            <a href="https://dashboard.stripe.com/test/customers/<?= htmlspecialchars($cust->id) ?>"
