...
$pageTitle = ($customerId ? 'Customer: ' . $customerId : 'Customer Details') . ' - Stripe TestKit';
...
<div class="mb-6 flex items-center justify-between">
    ...
    <a href="customers.php" class="inline-flex items-center gap-1 text-xs font-medium text-slate-600 hover:text-slate-900 bg-white border border-slate-200 px-3 py-1.5 rounded-lg shadow-xs transition">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 p-6 shadow-xs mb-8">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            <div class="flex items-start sm:items-center gap-4">
                <div class="w-14 h-14 rounded-2xl bg-stripe-50 text-stripe-600 border border-stripe-100 flex items-center justify-center text-xl font-bold uppercase shrink-0">
                    <?= htmlspecialchars(substr($customer->name ?? $customer->email ?? 'C', 0, 2)) ?>
                </div>
                <div>
                    <div class="flex flex-wrap items-center gap-2">
                        <h1 class="text-2xl font-bold text-slate-900"><?= htmlspecialchars($customer->name ?? 'Unnamed Customer') ?></h1>
                        <?php if ($customer->delinquent): ?>
                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold bg-rose-50 text-rose-700 border border-rose-200">Delinquent</span>
                        <?php else: ?>
                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-emerald-50 text-emerald-700 border border-emerald-200">Good Standing</span>
                        <?php endif; ?>
                    </div>
                    <div class="flex flex-wrap items-center gap-3 mt-1.5 text-xs text-slate-500">
                        <div class="flex items-center gap-1 font-mono text-slate-700 bg-slate-100 px-2 py-0.5 rounded">
                            ...
                            <button type="button" onclick="copyToClipboard('<?= htmlspecialchars($customer->id) ?>', this)" class="text-slate-400 hover:text-slate-700" title="Copy Customer ID">
            ...
            <div class="flex flex-wrap items-center gap-2.5">
                <a href="transactions.php?customer=<?= urlencode($customer->id) ?>" class="inline-flex items-center gap-2 bg-stripe-50 hover:bg-stripe-100 text-stripe-700 border border-stripe-200 text-xs font-semibold px-3.5 py-2 rounded-lg transition shadow-xs">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/></svg>
                    Filter All Transactions
                </a>
                <a href="subscriptions.php?customer=<?= urlencode($customer->id) ?>" class="inline-flex items-center gap-2 bg-purple-50 hover:bg-purple-100 text-purple-700 border border-purple-200 text-xs font-semibold px-3.5 py-2 rounded-lg transition shadow-xs">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
                    Filter All Subscriptions
                </a>
                <a href="https://dashboard.stripe.com/test/customers/<?= htmlspecialchars($customer->id) ?>" target="_blank" rel="noreferrer" class="inline-flex items-center gap-1.5 bg-white hover:bg-slate-50 text-slate-700 border border-slate-300 text-xs font-semibold px-3.5 py-2 rounded-lg transition shadow-xs">
                    Stripe Dashboard
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mt-6 pt-6 border-t border-slate-100 text-xs">
            <div>
                ...
                <div class="font-semibold text-slate-800 text-sm mt-0.5 truncate"><?= htmlspecialchars($customer->email ?? '—') ?></div>
            </div>
            <div>
                <span class="text-slate-400 font-medium">Phone Number</span>
                <div class="font-semibold text-slate-800 text-sm mt-0.5"><?= htmlspecialchars($customer->phone ?? '—') ?></div>
            </div>
            <div>
                <span class="text-slate-400 font-medium">Account Balance</span>
                <div class="font-semibold text-sm mt-0.5 <?= ($customer->balance ?? 0) < 0 ? 'text-emerald-600' : (($customer->balance ?? 0) > 0 ? 'text-rose-600' : 'text-slate-800') ?>">
                    <?= htmlspecialchars(formatCurrency($customer->balance ?? 0, $customer->currency ?? 'usd')) ?>
                    <span class="text-slate-400 text-2xs uppercase ml-0.5"><?= htmlspecialchars($customer->currency ?? 'usd') ?></span>
                </div>
            </div>
            <div>
                <span class="text-slate-400 font-medium">Description</span>
                <div class="font-semibold text-slate-800 text-sm mt-0.5 truncate"><?= htmlspecialchars($customer->description ?? '—') ?></div>
            </div>
        </div>
    </div>
                ...
                <div class="p-5">
                    <?php if (empty($paymentMethods)): ?>
                        <div class="text-center py-6 text-slate-400 text-xs">
                            <svg class="w-8 h-8 mx-auto mb-2 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/></svg>
                    ...
                    <?php else: ?>
                        <div class="space-y-3">
                            <?php foreach ($paymentMethods as $pm): ?>
                                ...
                                <div class="flex items-center justify-between p-3 rounded-lg border border-slate-200 bg-slate-50/50 hover:bg-slate-50 transition">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-7 rounded bg-white border border-slate-200 flex items-center justify-center font-bold text-xs uppercase text-slate-700 shadow-xs">
                                        ...
                                        <div>
                                            ...
                                            <div class="text-2xs text-slate-400">
                                                Exp: <?= htmlspecialchars((string)($card->exp_month ?? '')) ?>/<?= htmlspecialchars((string)($card->exp_year ?? '')) ?> • <span class="capitalize"><?= htmlspecialchars($card->funding ?? '') ?></span>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="text-right">
                                        <?php if (($customer->invoice_settings->default_payment_method ?? null) === $pm->id): ?>
                                            <span class="inline-flex items-center px-2 py-0.5 rounded text-2xs font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200">Default</span>
                                        <?php endif; ?>
                                        <div class="font-mono text-3xs text-slate-400 mt-1"><?= htmlspecialchars(substr($pm->id, 0, 14)) ?>...</div>
                                    </div>
                                </div>
                            <?php endforeach; ?>
                        </div>
                ...
                <div>
                    ...
                    if ($addr && ($addr->line1 || $addr->city || $addr->country)): ?>
                        <div class="text-slate-700 leading-relaxed font-medium">
                            ...
                            <?= htmlspecialchars($addr->city ?? '') ?><?= !empty($addr->state) ? ', ' . htmlspecialchars($addr->state) : '' ?> <?= htmlspecialchars($addr->postal_code ?? '') ?><br>
                            <span class="uppercase font-bold text-2xs text-slate-500"><?= htmlspecialchars($addr->country ?? '') ?></span>
                        </div>
                    <?php else: ?>
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden">
                <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                    ...
                    <a href="subscriptions.php?customer=<?= urlencode($customer->id) ?>" class="text-xs font-semibold text-stripe-600 hover:text-stripe-700 hover:underline">
                                    ...
                                    <tr class="hover:bg-slate-50/75 transition">
                                        <td class="px-6 py-3 font-mono font-medium text-slate-800">
                                            <div class="flex items-center gap-1.5">
                                                ...
                                                <button type="button" onclick="copyToClipboard('<?= htmlspecialchars($sub->id) ?>', this)" class="text-slate-400 hover:text-slate-700" title="Copy Subscription ID">
                                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>
                                                </button>
                                            </div>
                                        </td>
            ...
            <div class="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden">
                <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                    ...
                    <a href="transactions.php?customer=<?= urlencode($customer->id) ?>" class="text-xs font-semibold text-stripe-600 hover:text-stripe-700 hover:underline">
                                    ...
                                    <tr class="hover:bg-slate-50/75 transition">
                                        <td class="px-6 py-3 font-mono font-medium text-slate-800">
                                            <div class="flex items-center gap-1.5">
                                                ...
                                                <button type="button" onclick="copyToClipboard('<?= htmlspecialchars($ch->id) ?>', this)" class="text-slate-400 hover:text-slate-700" title="Copy Charge ID">
                                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>
                                                </button>
                                            </div>
                                        </td>
                                        <td class="px-6 py-3">
                                            <span class="font-bold text-slate-900"><?= htmlspecialchars(formatCurrency($ch->amount, $ch->currency)) ?></span>
                                            <?php if ($ch->refunded): ?>
